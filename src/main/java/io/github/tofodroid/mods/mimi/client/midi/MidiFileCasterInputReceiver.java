...
public class MidiFileCasterInputReceiver extends MidiInputReceiver {
    ...
    public void sendTransmitterNoteOnPacket(Byte channel, Byte midiNote, Byte velocity) {
        if(MIMIMod.proxy.isClient()) {
            TransmitterNotePacket packet = TransmitterNotePacket.createNotePacket(channel, midiNote, velocity, ((ClientProxy)MIMIMod.proxy).getMidiInput().getPublicTransmit());
    ...
    public void sendTransmitterNoteOffPacket(Byte channel, Byte midiNote) {
        if(MIMIMod.proxy.isClient()) {
            TransmitterNotePacket packet = TransmitterNotePacket.createNotePacket(channel, midiNote, Integer.valueOf(0).byteValue(), ((ClientProxy)MIMIMod.proxy).getMidiInput().getPublicTransmit());
            NetworkManager.BROADCAST_CHANNEL.sendToServer(packet);
        }
    }
    ...
    public void sendTransmitterAllNotesOffPacket(Byte channel) {
        if(MIMIMod.proxy.isClient()) {
            TransmitterNotePacket packet = TransmitterNotePacket.createAllNotesOffPacket(channel, ((ClientProxy)MIMIMod.proxy).getMidiInput().getPublicTransmit());
            NetworkManager.BROADCAST_CHANNEL.sendToServer(packet);
        }
    }
    ...
    public void sendTransmitterControllerPacket(Byte channel, Byte controller, Byte value) {
        if(MIMIMod.proxy.isClient()) {
            TransmitterNotePacket packet = TransmitterNotePacket.createControllerPacket(channel, controller, value, ((ClientProxy)MIMIMod.proxy).getMidiInput().getPublicTransmit());
            NetworkManager.BROADCAST_CHANNEL.sendToServer(packet);
        }
    }
