...
public class MidiDeviceInputReceiver implements Receiver {
    ...
    protected void handleMessage(ShortMessage message) {
        ...
        if(player != null && MIMIMod.getProxy().isClient()) {
            BroadcastEvent event = BroadcastEvent.fromShortMessage(message, player.getUUID(), player.level().dimension(), EntityUtils.getEntityHeadPos(player), 16, MathUtils.addClamped(message.getData2(), ConfigProxy.getMidiDeviceVelocity(), 0, 127));
