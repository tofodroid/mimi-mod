...
public class JEIPlugin implements IModPlugin {
	...
	public void registerRecipes(IRecipeRegistration registration) {
		...
		registration.addRecipes(TuningTableRecipeCategory.RECIPE_TYPE, manager.getRecipes().parallelStream().filter(recipe -> 
            recipe.value().getType().equals(ModRecipes.TUNING_TYPE)).map(r -> (TuningTableRecipe) r.value()).collect(Collectors.toList())
        );
	}
