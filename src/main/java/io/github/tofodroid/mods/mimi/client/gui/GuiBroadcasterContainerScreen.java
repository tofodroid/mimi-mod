...
public class GuiBroadcasterContainerScreen extends BaseContainerGui<ContainerBroadcaster> {
    ...
    protected GuiGraphics renderGraphics(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        // Transmit Screen    
        graphics.blit(guiTexture, START_X + Float.valueOf(TRANSMIT_SCREEN.x()).intValue(), START_Y + Float.valueOf(TRANSMIT_SCREEN.y()).intValue(), 1 + (13 * (this.menu.getBroadcasterTile().isPublicBroadcast() ? 0 : 1)), 231, 13, 13, TEXTURE_SIZE, TEXTURE_SIZE);
